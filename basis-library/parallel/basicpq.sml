(* Modification of basicpd.sml for the newer prompt scheduling algorithm using
 * priority queues for multiple priority levels.
 * 
 * At the moment, this is really just an unedited copy of basicpd.sml.
 * TODO... *)
...
(* SAM_NOTE: is this for debugging only? *)
...
fun popBottom  (q as Queue {top, bottom, work, ... }) =
    ...
    in
        if i = j then raise WorkQueue (* empty *)
            ...
            (top := i - 1;
             (* SAM_NOTE: I'm guessing `ignore` is just used to make the type
              * checker happy when sequencing functions that don't return
              * unit? *)
             (if debug then ignore (fetchAndAdd (tasksAdded, ~1)) else ());
             case A.sub (w, i - 1) of
                 Empty => raise WorkQueue
