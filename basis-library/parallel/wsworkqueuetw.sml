...
functor WorkQueue (W : sig type work val numberOfProcessors : unit -> int end)
        : PARALLEL_WORKQUEUE =
*)
functor WorkQueue (structure W : sig type work val numberOfProcessors : unit -> int end
                   structure P : POLICY) =
struct
...
(* move queue at a into p', assumes the master lock is held *)
  ...
  fun getWork p =
      (* uncomment for prompt getWorkN numberOfProcessors p  *)
      getWorkLat (P.workOnLatency numberOfProcessors p) p
