...
and entry = Empty | Work of (token * job * P.t (* * int *))
and queue = Queue of
            {
              top : int ref,
              bottom : int ref,
...
val queues = A.tabulate (P, fn _ => newQueue ())
...
fun add (q as Queue {top, bottom, work, ... }) (tw as (t, w, prio)) =
    let
...
fun dealAttempt p =
    let val _ = assertblocked "dealAttempt"
        val (q as Queue {top, bottom, work, ...}) = A.sub (queues, p)
        val i = !top
        val j = !bottom
        val w = !work
        val p' =
        ...
        val vv = !c
        fun searchForBetter min max prio =
            if min > max then NONE
            else
                let val Work (mt, mw, mp) = A.sub (w, min)
                in
                    if prio p< mp then
                        Work (mt, mw, mp)
                    else
                        searchForBetter (min + 1) max prio
                end
        fun haveBetter pri =
            if i - j <= 1 then NONE
            else
                searchForBetter j i (prio_of_flag pri)
    in
        if p' = p orelse vv >= notwaiting then false
        else
...
val policyName = "Prompt private deques"
  ...
  fun resumeInt p (Suspend (prio, k), v) =
      ...
      in
        (resumeWork (p, (newWork p, Thread t, prio))
    ...
    | resumeInt p (Capture (prio, k), v) =
      ...
      in
        addWorkLat (p, [(newWork p, Thread (T.prepend (k, fn () => v)), prio)])
...
*)
  ...
  val () = MLton.Parallel.registerProcessorFunction (prun)
  val () = init ()
