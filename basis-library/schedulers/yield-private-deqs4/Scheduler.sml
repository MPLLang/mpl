...
struct
  ...
  val lockTestAndSet = _import "Parallel_lockTestAndSet64" runtime private : int ref * int -> int;
      ...
      fun push (t, v) =
        ...
        in
          if chld = ~1 then
            ...
            let
              val _ = Queue.pushBot ((t, v), myQueue)
              (* val mail = 
                  SOME (fn () => (t (); print("thd "^(Int.toString chld)^" exe task\n"); returnToSched v)) *)
              val mail = 
                case Queue.popTop myQueue of
                  NONE => NONE
                | SOME (task, v) =>
        ...
        if cnt >= (YIELD_CNT-1) then
          ...
          in
            ...
            then (verifyStatus (); 
              ...
              let val (recv, mail) = Mailboxes.getMail mailboxes myId (requestCell victimId)
              in
                if recv then 
                  case mail of
                    NONE => (verifyStatus ();
