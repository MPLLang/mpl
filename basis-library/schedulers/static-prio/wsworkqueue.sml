...
sig
  ...
  (* workonLatency n proc returns what priority proc should work on
     when there are n processors *)
  val workOnPriority : int -> int -> Priority.t
...
*)
  ...
  val queues : queue A.array ref = ref (A.fromList [])

  fun queueIndex (p, r) =
      p * (Priority.count ()) + (Priority.toInt r) - 1

  fun indPrio i =
      Priority.fromInt ((i mod (Priority.count ())) + 1)

  fun initQueues () =
      queues := A.tabulate (numberOfProcessors * (Priority.count ()),
                            fn i => newQueue (indPrio i) i)

  fun queue (p, r) =
      (A.sub (!queues, queueIndex (p, r)))
      handle Subscript => (print "queue\n"; raise Subscript)
...
*)
  ...
  fun addWork (prio, p, tws) =
    let
      ...
      val q as Queue { top, work, ... } = queue (p, prio)
...
(* move queue at a into p', assumes the master lock is held *)
  fun moveQueue prio (a, p') = ()
                                   (*
      if a = p' then ()
      else
        let
            val q as Queue { index, ... } = queue (
                case A.sub (queue prio, a) of
                      SOME q => q
                    | NONE => (print "move\n"; raise WorkQueue)
        in
          index := p';
          A.update (queue prio, p', SOME q)
        end
*)
  ...
  fun getWork p =
    let
      ...
      (* val () = pr p "before-get" *)
        val prio = P.workOnPriority numberOfProcessors p
        (* val _ = print ((Int.toString p) ^ " " ^ (Priority.toString prio) ^ "\n") *)
    in
        (* if lat then (print "Hi\n"; usleep 1000000; getWork p)  else *)
        let
            val q as Queue { bottom, top, work, index, ... } = queue (p, prio)
      val Lock { ownerLock = lock, thiefLock, ... } = A.sub (locks, p)
                   ...
                   case A.sub (!work, i - 1)
                    of Empty => raise WorkQueue
                     | Marker => (A.update (!work, i - 1, Empty);
                                  ...
                                  case getWork p
                                   of NONE => NONE
                                    | SOME (_, _, w) => SOME (true, prio, w))
                     | Work tw => SOME (false, prio, #2 tw)
                                  before (A.update (!work, i - 1, Empty);
                                          count p "get" ~1;
                                          (* dekkerUnlock (true, lock); *)
                                          releaseLock thiefLock  ) (* XTRA *))
      ...
      in
        ((*if P.suspendEntireQueues then
           ...
           in
             ...
             A.update (queues, p, (* XXX PERF? NONE *) SOME (newQueue prio p));
             count p "suspend" 0;
  ...
  fun resumeWork (prio, p, _ (* XXX *), tw as (t as Token r, w)) =
      let
      ...
      (* else (* make a new queue *)
        let
          ...
          val q as Queue { work, top, ... } = newQueue prio a
    ...
    | resumeWork (prio, p, q as SOME (Queue { work, top, index, ... }), tw as (t, w)) =
      let
...
(* what if a job J is added, then that queue is suspended, then resumed by a
  ...
  fun removeWork (p', Token r) =
      let
        ...
        val Queue { index, ... } = case !r of SOME q => q | NONE => raise WorkQueue
(* this is a litte SUSP -- what if the queue is moved while we are trying to lock it? *)
        ...
        val Queue { top, bottom, work, ... } = case !r of SOME q => q | NONE => raise WorkQueue
        val last = !top - 1
        val j = !bottom
  ...
  val workOnPriority = P.workOnPriority
...
struct
  ...
  local val arr = Array.array (1024, NONE)
  in
  fun workOnPriority n p =
      case Array.sub (arr, p) of
          SOME r => r
        | NONE =>
          let val p' = n - p - 1
              val r = Priority.chooseFromDist ((real p') / (real n))
          in
              Array.update (arr, p, SOME r);
              r
          end
  end
  val policyName = "ws6"
