...
struct
  ...
  fun workOnLatency n p = !globalLatency
  val policyName = "ws6"
end

  structure Q = WorkQueue (structure W =
                           struct
                             type work = job
  ...
  fun resume (Suspend (lat, k, q), v) =
      ...
      in
        (Q.resumeWork (lat, p, q, (Q.newWork p, Thread t))
  ...
  fun procio p =
      let (* val _ = if p = 0 then print "enter procio\n" else () *)
  ...
  fun interruptFst t =
      ...
      in
       ((* print ("interruptFst on " ^ (Int.toString p) ^ "\n"); *)
         globalLatency := not (!globalLatency);
        signalOthers p (Posix.Signal.usr1) 0;
        (if not lfw then Q.addWork (lat, p, [(Q.newWork p, RThread t)]) else ());
 ...
 (*  val _ = S.setHandler (sg, (S.Handler.ignore)) *)
  ...
  fun init () =
      let val p = processorNumber ()
          ...
          val iv = Time.fromMilliseconds 10
      in
          (if p = 0 then
