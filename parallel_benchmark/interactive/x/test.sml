...
(*				 MLX.setforeground d gc 0wx9933EE;
  ...
  | f ["mouse"] =
	...
	val _ = MLX.flush d
    fun loop () =
        let val (x, y) = MLX.mousepos d w
            val bs = MLX.buttons d w
            val b = MLX.button d w
            val _ = print ("Mouse: (" ^ (Int.toString x) ^ ", "
                           ^ (Int.toString y) ^ ")\n")
            val _ = if b then
                        print ((Int.toString (List.length bs)) ^ " buttons\n")
                    else
                        print "No buttons\n"
        in
            (MLX.usleep 1000; loop ())
        end
    in
        loop ()
    end
