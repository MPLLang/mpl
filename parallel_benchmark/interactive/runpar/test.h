#ifndef __TEST_ML_H__
#define __TEST_ML_H__
...
#if !defined(PART_OF_TEST) && \
    !defined(STATIC_LINK_TEST) && \
    !defined(DYNAMIC_LINK_TEST)
#define DYNAMIC_LINK_TEST
#endif

#if defined(PART_OF_TEST)
#define MLLIB_PRIVATE(x) PRIVATE x
#define MLLIB_PUBLIC(x) PUBLIC x
#elif defined(STATIC_LINK_TEST)
#define MLLIB_PRIVATE(x)
#define MLLIB_PUBLIC(x) PUBLIC x
#elif defined(DYNAMIC_LINK_TEST)
#define MLLIB_PRIVATE(x)
#define MLLIB_PUBLIC(x) EXTERNAL x
#else
#error Must specify linkage for test
#define MLLIB_PRIVATE(x)
...
MLLIB_PUBLIC(void test_open(int argc, const char** argv);)
MLLIB_PUBLIC(void test_close();)
