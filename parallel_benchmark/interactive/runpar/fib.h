...
#if !defined(PART_OF_FIB) && \
    !defined(STATIC_LINK_FIB) && \
    !defined(DYNAMIC_LINK_FIB)
#define DYNAMIC_LINK_FIB
#endif

#if defined(PART_OF_FIB)
#define MLLIB_PRIVATE(x) PRIVATE x
#define MLLIB_PUBLIC(x) PUBLIC x
#elif defined(STATIC_LINK_FIB)
#define MLLIB_PRIVATE(x)
#define MLLIB_PUBLIC(x) PUBLIC x
#elif defined(DYNAMIC_LINK_FIB)
#define MLLIB_PRIVATE(x)
#define MLLIB_PUBLIC(x) EXTERNAL x
#else
#error Must specify linkage for fib
#define MLLIB_PRIVATE(x)
...
MLLIB_PUBLIC(void fib_open(int argc, const char** argv);)
MLLIB_PUBLIC(void fib_close();)
