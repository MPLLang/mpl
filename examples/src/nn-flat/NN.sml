...
sig
  ...
  val makeTree : int -> point seq_flat -> t
...
struct
  ...
  fun makeTreeBounded leafSize (verts : point seq_flat) (idx : int seq) ((xLeft, yBot) : G.point) width =
    if AS.length idx <= leafSize then
      ...
      val ((sorted, offsets), tm) = Util.getTime (fn () =>
        CountingSort.sort idx (fn i =>
          G.quadrant center (SeqFlat.nth verts (Seq.nth idx i))) 4)
  ...
  fun makeTree leafSize (verts : point seq_flat) =
    if AFS.length verts = 0 then raise Fail "makeTree with 0 points" else
    let
      (* calculate the bounding box *)
      fun maxPt ((x1,y1),(x2,y2)) = (Real.max (x1, x2), Real.max (y1, y2))
      fun minPt ((x1,y1),(x2,y2)) = (Real.min (x1, x2), Real.min (y1, y2))
      fun getPt i = AFS.sub (verts, i)
      val (xLeft,yBot) = reduce 10000 minPt (Real.posInf, Real.posInf) (getPt, 0, AFS.length verts)
      val (xRight,yTop) = reduce 10000 maxPt (Real.negInf, Real.negInf) (getPt, 0, AFS.length verts)
      val width = Real.max (xRight-xLeft, yTop-yBot)

      val idx = Seq.tabulate (fn i => i) (AFS.length verts)
