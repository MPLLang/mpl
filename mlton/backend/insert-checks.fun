...
fun restrictAllocInBlock
  ...
  case Transfer.bytesAllocated originalTransfer of
    ...
    BytesAllocated.Dynamic _ => [originalBlock]
    (* Otherwise we take into account the bytes needed for the transfer. *)
  | BytesAllocated.Static transferBytes =>
...
fun limitCheckCoalesce (f: Function.t, maxAlloc, tyconTy) =
   let
       ...
       * `blockLimitCheckAmount` behaves as a function. *)
            ...
            val (f, (blockLimitCheckAmount, ensureFree)) =
               case !Control.limitCheck of
                  PerBlock => (*(f, limitCheckPerBlock (f, tyconTy))*)
                     Error.bug "LimitCheck.transform: PerBlock not supported"
                | _ => let
                           ...
                           *)
                          val maxBytesAlloc = Bytes.- (Bytes.fromInt (512*7), Runtime.limitSlop)
                          val f = restrictAllocInEachBlock (f, maxBytesAlloc, tyconTy)
                       in
                          (f, limitCheckCoalesce (f, maxBytesAlloc, tyconTy))
                       end
