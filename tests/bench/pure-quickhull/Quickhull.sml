structure Quickhull :
sig
  val hull : (real * real) PureSeq.t -> int PureSeq.t
end =
struct
  ...
  fun hull pts =
    let
      ...
      fun parHull idxs l r =
        if PureSeq.length idxs < 2 then
          Tree.fromPureSeq idxs
        (* if DS.length idxs <= 2048 then
          ...
          let
            ...
            val (mid, _) = SeqBasis.reduce 10000 max (~1, Real.negInf)
              (0, PureSeq.length idxs)
              (fn i => (PureSeq.nth idxs i, d (PureSeq.nth idxs i)))
      ...
      val (above, below) =
        ...
        Split.parSplit
          (PureSeq.tabulate (fn i => i) (PureSeq.length pts))
          (PureSeq.tabulate flag (PureSeq.length pts))
      ...
      val result = Tree.toPureSeq hullt
